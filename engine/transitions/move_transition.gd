...
func start_transition(from_slide: Slide, to_slide: Slide) -> Tween:
	...
	var previous_slide_target_pos: Vector2 = visible_pos + (SlideHelper.get_context().slide_size * slide_move_direction)
	var next_slide_source_pos: Vector2 = visible_pos + (SlideHelper.get_context().slide_size * -slide_move_direction)
