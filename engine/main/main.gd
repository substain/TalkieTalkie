...
@export var ui: UI
@export var slideshow_timer: Timer
...
func _ready() -> void:	
	...
	# TODO do we?  (we need to defer this because some initialization happens at the end of the first frame)
	setup_initial_state()
...
func do_continue(automatic: bool = false) -> void:
	...
	transition_to_slide(previous_slide, current_slide, used_transition.start_transition)
