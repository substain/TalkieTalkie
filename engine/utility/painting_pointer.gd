...
## Backup icon
const CIRCLE_1X1: Texture2D = preload("res://style/ui/circle_1x1.svg")
const PAINT_GRADIENT: Gradient = preload("res://style/ui/paint_gradient.tres")
...
func _process(_delta: float) -> void:
	...
	SlideHelper.pointing_at_pos.emit(target_pos, is_drawing, paint_props)
...
func check_pointing(event: InputEvent) -> void:
	...
	if event.is_action_pressed("draw_pointer_1"):
		...
		is_pointing = true
	elif event.is_action_pressed("draw_pointer_2"):
		current_da_index = DP_2_INDEX
		is_pointing = true

	elif event.is_action_released("draw_pointer_1") && current_da_index == DP_1_INDEX:
		is_pointing = false
		SlideHelper.stop_drawing.emit()

	elif event.is_action_released("draw_pointer_2") && current_da_index == DP_2_INDEX:
		is_pointing = false
		SlideHelper.stop_drawing.emit()
