class_name TalkieDrawPointer
extends Control

const MIN_DRAW_DIST: float = 5.0

## Backup icon
const _CIRCLE_1X1: Texture2D = preload("uid://dfgyj4wldg65x") #/style/ui/circle_1x1.svg
const _DRAW_GRADIENT: Gradient = preload("uid://cudd2yfejk5jn") #/style/ui/draw_gradient.tres
...
@export var all_draw_properties: Array[TalkieDrawProperties] = []
...
func _ready() -> void:
	...
	for draw_prop: TalkieDrawProperties in all_draw_properties:
...
func _process(_delta: float) -> void:
	...
	if !is_pointing:
		return
	var draw_props: TalkieDrawProperties = all_draw_properties[current_da_index]
	var target_pos: Vector2 = get_global_mouse_position()
	icon_rect.global_position = target_pos + icon_offset
	TalkieSlideHelper.pointing_at_pos.emit(target_pos, is_drawing, draw_props)
...
func check_pointing(event: InputEvent) -> void:
	...
	if event.is_action_pressed("tt_draw_pointer_1"):
		...
		is_pointing = false
		TalkieSlideHelper.stop_drawing.emit()
...
func check_drawing(event: InputEvent) -> void:
	var p_props: TalkieDrawProperties = all_draw_properties[current_da_index]
...
func create_new_line(p_props: TalkieDrawProperties) -> void:
	if is_instance_valid(current_line):
		managed_lines.append(current_line)
...
func _on_slide_changed(_new_slide: TalkieSlide) -> void:
	is_drawing = false
...
static func get_line(p_props: TalkieDrawProperties) -> Line2D:
