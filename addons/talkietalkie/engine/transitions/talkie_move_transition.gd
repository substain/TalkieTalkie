...
func start_transition(from_slide: TalkieSlide, to_slide: TalkieSlide) -> Tween:
	if !TalkieUtil.has_position_property(from_slide):
		...
		return
	if !TalkieUtil.has_position_property(to_slide):
		push_warning("MoveTransition expects both slides to have a 'position' property, but '", from_slide, "' does not have one. Stopping the transition.")
		return
	...
	var previous_slide_target_pos: Vector2 = visible_pos + (TalkieSlideHelper.get_context().slide_size * slide_move_direction)
	var next_slide_source_pos: Vector2 = visible_pos + (TalkieSlideHelper.get_context().slide_size * -slide_move_direction)
...
func _clean_up(from_slide: TalkieSlide, to_slide: TalkieSlide) -> void:
