...
const MD_FORMAT_BASICS_LFR: Array[TalkieLineFormatRule] = [
	## make sure links are processed first, because they may interact with bbcode
	# /engine/resources/link_lfr.tres
	preload("uid://r70278nk01s4"), # [test](target) -> [url=target]test[/url]    
	
	# /engine/resources/bullet_point_lfr.tres
	preload("uid://dfe6wjonsvave"), # * -> •
	
	# /engine/resources/bold_italics_lfr.tres
	preload("uid://iuh60ggb8x03"), # ***test*** -> [b][i]test[/i][/b]
	
	# /engine/resources/bold_lfr.tres
	preload("uid://jxo6rq4vmxvk"), # **test** -> [b]test[/b]
	
	# /engine/resources/italics_lfr.tres
	preload("uid://ckg83eqow5mqi"), # *test* -> [i]test[/i]
	
	# /engine/resources/code_lfr.tres
	preload("uid://bjxbfdv474ane"), # `test` -> [code]test[/code]
	
	# /engine/resources/strikethrough_lfr.tres
	preload("uid://bcjp6hfcabsic"), # ~~test~~ -> [s]test[/s]
	
	# /engine/resources/underline_lfr.tres
	preload("uid://cleasv3u4sshm") # ***test*** -> [b][i]test[/i][/b]
]
...
@export var line_format_rules: Array[TalkieLineFormatRule] = []
...
func do_generate() -> void:
	...
	var _slides: Array[TalkieSceneSlide] = _instantiate_as_slides(slide_infos)

	#for slide_info: SlideInfo in slide_infos:
		#TalkieUtil.tt_debug(_to_debug_string(slide_info))

func _instantiate_as_slides(slide_infos: Array[SlideInfo]) -> Array[TalkieSceneSlide]:
	if slide_scene == null:
		TalkieUtil.tt_warn("Cannot instantiate a null slide scene. Ensure slide_scene is set in the edtior.")
		return []
		
	_generated_node_names = []
	var res: Array[TalkieSceneSlide] = []
	for slide_info: SlideInfo in slide_infos:
		var slide: TalkieSceneSlide = _instantiate_slide(slide_info)
		if slide != null:
			res.append(slide)
		
	return res

func _instantiate_slide(slide_info: SlideInfo) -> TalkieSceneSlide:
	...
	if !_generated_node_names.has(slide_node_name) && target_parent_node.has_node(slide_node_name):
		if ignore_if_slide_name_exists:
			TalkieUtil.tt_printr("Will not create a new slide for '%s', a node with that name already exists." % slide_node_name)
			return null
		if replace_existing_instantiated_slides:
			TalkieUtil.tt_printr("Replacing existing node '%s' with a newly generated slide." % slide_node_name)
			target_parent_node.get_node(slide_node_name).free()
	
	var slide: TalkieSceneSlide = slide_scene.instantiate(PackedScene.GEN_EDIT_STATE_INSTANCE) as TalkieSceneSlide
	target_parent_node.add_child(slide)
	slide.name = slide_node_name
	_generated_node_names.append(slide.name)
	TalkieUtil.make_instantiated_scene_local(slide, get_tree().edited_scene_root)
...
static func _set_title(instantiated_slide: TalkieSceneSlide, new_title: String, title_node_path: String) -> void:
	instantiated_slide.slide_title = new_title

	if title_node_path == null || !instantiated_slide.has_node(title_node_path):
		TalkieUtil.tt_warn("Could not find title node path '%s' in the instantiated scene. Will not set a title in the created slide." % title_node_path)
		return
	
	var title_node: Node = instantiated_slide.get_node(title_node_path)
	if !TalkieUtil.has_text_property(title_node):
		TalkieUtil.tt_warn("Can only set title text on a control with a text property, but '%s' does not have a text property. Setting a title will be skipped." % title_node.name)
		return
	
	@warning_ignore("unsafe_property_access") # We already enssured that property exists.
	title_node.text = new_title

static func _set_contents(instantiated_slide: TalkieSceneSlide, new_contents: Array[String], content_parent_node_path: String, content_scene_to_use: PackedScene, do_combine_content_lines: bool) -> void:
	instantiated_slide.slide_content = "\n".join(new_contents)
	if content_parent_node_path == null || !instantiated_slide.has_node(content_parent_node_path):
		TalkieUtil.tt_warn("Could not find content parent node path '%s' in the instantiated scene. Will not set contents in the created slide." % content_parent_node_path)
		return
	...
	if content_scene_to_use == null:
		TalkieUtil.tt_warn("Cannot instantiate null content scenes. Created slides will have no content. Ensure content_scene_to_use is set in the edtior.")
		return
...
static func _create_content_node(content_parent: Node, content_scene_to_use: PackedScene, content_text: String) -> void:
	...
	TalkieUtil.make_instantiated_scene_local(content_node, content_parent.get_tree().edited_scene_root)
	content_node.set_display_folded(true)
	if !TalkieUtil.has_text_property(content_node):
		TalkieUtil.tt_warn("can only set content text on a control with a text property, but '%s' does not have a text property. Setting contents will be skipped." % content_node.name)
		return
...
func _process_header_line(line: String) -> bool:
	...
	TalkieUtil.tt_debug("line '%s' is header depth %s"  % [line,str(md_header_match.get_string().count("#"))])
	return true
...
func _format_string_by_rules(line: String) -> String:
	...
	if use_markdown_format_basics:
		for rule: TalkieLineFormatRule in MD_FORMAT_BASICS_LFR:
			res = rule.format(res)
	
	for rule: TalkieLineFormatRule in line_format_rules:
		res = rule.format(res)
	return res
