...
const _PRESENTATION_BASE_SCENE: PackedScene = preload("uid://c4fiin221skal") #/engine/base/presentation.tscn
const _DEFAULT_TITLE_LAYOUT_SCENE: PackedScene = preload("uid://ddf7utr1vtc3d") #/engine/default_template/default_title_layout.tscn
const _DEFAULT_TEXT_LAYOUT_SCENE: PackedScene = preload("uid://cvtb6gj6itur4") #/engine/default_template/default_text_layout.tscn
const _DEFAULT_CONTENT_ELEMENT_SCENE: PackedScene = preload("uid://u5p5x65m3ulh") #/engine/default_template/default_content_element.tscn

const _SLIDE_GENERATOR_SCRIPT: Script = preload("uid://deou7up6h48pg") #/engine/generation/slide_generator.gd

const _2D_PRESENTATION_SCRIPT: Script = preload("uid://vhq5381tygcu") #/engine/base/presentation_2d.gd
const _3D_PRESENTATION_SCRIPT: Script = preload("uid://c3453y23ha8re") #/engine/base/presentation_3d.gd

const _2D_LINE_MAPPER_SCRIPT: Script = preload("uid://dxc3enjoi52u6") #/engine/mapper/slide_2d_line_mapper.gd
const _3D_LINE_MAPPER_SCRIPT: Script = preload("uid://by3b6esejhnuo") #/engine/mapper/slide_3d_line_mapper.gd

const _BULLET_POINT_LFR = preload("uid://dfe6wjonsvave") #/engine/resources/bullet_point_lfr.tres

const _SLIDE_PARENT_NODE_NAME: String = "Slides"

static var _PRESENTATION_BASE_PATH: String = TalkieSetup.get_addon_path() + "/engine/base/talkie_presentation.tscn"
...
static func do_generate(pg_data: PresentationGenerationData, do_save: bool = true) -> void:
	...
	setup_presentation(pg_data, EditorInterface.get_edited_scene_root() as TalkiePresentation)	
...
static func setup_presentation(pg_data: PresentationGenerationData, presentation: TalkiePresentation) -> void:
	...
	var ui: TalkieUI = presentation.ui
	var slide_controller: TalkieSlideController = presentation.slide_controller
	...
	if pg_data.presentation_type == PresentationType.CONTROL:
		...
		add_node_to_scene(slide_parent_node, presentation, presentation, _SLIDE_PARENT_NODE_NAME)
		...
		presentation.set_script(_2D_PRESENTATION_SCRIPT)
		presentation.ui = ui
		...
		## add a camera2D and reference it in the presentation2D
			...
			(presentation as TalkiePresentation2D).camera = camera_2d

		## prepare a slide 2D line mapper
		var slide_mapper_2d: Node2D = Node2D.new()
		slide_mapper_2d.set_script(_2D_LINE_MAPPER_SCRIPT)
		add_node_to_scene(slide_mapper_2d, presentation, presentation, "Slide2DMapper")

		## add the slide_parent_node as base for the slides
		add_node_to_scene(slide_parent_node, slide_mapper_2d, presentation, _SLIDE_PARENT_NODE_NAME)
		(slide_mapper_2d as TalkieSlide2DLineMapper).target_parent = slide_parent_node
		...
		presentation.set_script(_3D_PRESENTATION_SCRIPT)
		presentation.ui = ui
		presentation.slide_controller = slide_controller
		...
		if pg_data.add_camera_node:
			...
			(presentation as TalkiePresentation3D).camera = camera_3d
			
		## prepare a slide 3D line mapper
		var slide_mapper_3d: Node3D = Node3D.new()
		slide_mapper_3d.set_script(_3D_LINE_MAPPER_SCRIPT)
		add_node_to_scene(slide_mapper_3d, presentation, presentation, "Slide3DMapper")

		## add the slide_parent_node as base for the slides
		add_node_to_scene(slide_parent_node, slide_mapper_3d, presentation, _SLIDE_PARENT_NODE_NAME)
		(slide_mapper_3d as TalkieSlide3DLineMapper).target_parent = slide_parent_node
		#slide_base_node.set_process(true) #we don't actually need this now, but we may if we want to add _process() to this later
...
static func create_slide_content(pg_data: PresentationGenerationData, presentation: TalkiePresentation, slide_parent_node: Node) -> void:

	## add slide generator to use either right now or later on
	var slide_generator_node: Node = Node.new()
	slide_generator_node.set_script(_SLIDE_GENERATOR_SCRIPT)
	(slide_generator_node as SlideGenerator).line_format_rules = [_BULLET_POINT_LFR]
	...
	if pg_data.create_title_slide:
		...
		var title_slide: TalkieSceneSlide = _DEFAULT_TITLE_LAYOUT_SCENE.instantiate(PackedScene.GEN_EDIT_STATE_INSTANCE) as TalkieSlide
		add_node_to_scene(title_slide, slide_parent_node, presentation, "SlideTitle")
		TalkieUtil.make_instantiated_scene_local(title_slide, presentation) #get_tree().edited_scene_root
	...
	if pg_data.create_content_from_md && !pg_data.content_md_text.is_empty():
		...
		slide_generator.slide_scene = _DEFAULT_TEXT_LAYOUT_SCENE
		slide_generator.content_scene = _DEFAULT_CONTENT_ELEMENT_SCENE
		
		## generate slides from markdown
		slide_generator.do_generate()

static func add_background(pg_data: PresentationGenerationData, presentation: TalkiePresentation) -> void:
