@tool
class_name TalkieLineFormatRuleRegex
...
func format(line: String) -> String:
	...
	TalkieUtil.tt_debug("(LineFormatRuleRegex) replacing %s matches in '%s'" % [search_matches.size(), line])
	var res: String = line
	...
	for rgmatch: RegExMatch in search_matches:
		...
		TalkieUtil.tt_debug("(LineFormatRuleRegex) io: %s: rgm(%s) at (%s|%s). changed '%s' to '%s'"% [index_offset, rgmatch.get_group_count(), rgmatch.get_start(), rgmatch.get_end(), res_before_debug, res])
...
static func get_replaced_string(rgmatch: RegExMatch, replacements: Array[String]) -> String:
	...
	for rp_idx: int in replacements.size():
		...
		if first_idx == -1:
			TalkieUtil.tt_warn("(LineFormatRuleRegex) could not find capture group for '%s' in '%s' from index %s" % [ m,repl_string,current_search_index])
		...
		TalkieUtil.tt_debug("replaced '%s' for '%s' in target string: '%s'" % [m,replacements[rp_idx],repl_string])
...
func _init_regex() -> void:
	if _regex == null || _used_search_regex != search_regex:
		...
		if error_status != OK:
			TalkieUtil.tt_warn("(LineFormatRuleRegex) Could not compile '%s' : Error = %s" % [search_regex,error_status])
		else:
