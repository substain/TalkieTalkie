class_name TalkieTimeView
extends Control

## The (default) settings of this UI. May be overwritten via SideWindowBase
@export var settings: TimeViewSettings
...
var settings_override_active: bool = false
...
func _ready() -> void:
	...
	if settings.autostart_passed_time_timer:
		start_timer()

func _physics_process(_delta: float) -> void:
	if settings.show_clock_time:
		update_clock_time()
		
	if settings.show_passed_time && timer_running:
		update_passed_time()

	if settings.show_time_left && timer_running:
		update_time_left()
...
func override_settings(settings_override: TimeViewSettings) -> void:
	if settings_override == null:
		return
	settings = settings_override
	settings_override_active = true
	start_time_ms = Time.get_ticks_msec()
	if !settings.autostart_passed_time_timer:
		timer_running = false
	update_settings()
...
func update_settings() -> void:
	target_duration_ms = minutes_to_ms(settings.target_duration_minutes)
	clock_time_parent.visible = settings.show_clock_time
	target_duration_parent.visible = settings.show_target_duration
	passed_time_parent.visible = settings.show_passed_time
	time_left_parent.visible = settings.show_time_left
