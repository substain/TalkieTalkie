...
var target_audio_bus: String = ""

func _ready() -> void:
	super()

	target_audio_bus = ProjectSettings.get_setting("talkietalkie/settings/show_audio_for_bus", "Master") as String
		
	overall_volume_v_box.visible = !target_audio_bus.strip_edges().is_empty()
	lang_v_box.visible = ProjectSettings.get_setting("talkietalkie/settings/show_translation_control", true) as bool

	overall_volume_slider.set_value_no_signal(TalkiePreferences.audio_volume)
	lang_option_button.selected = get_locale_button_id(TalkiePreferences.language)
	about_button.text = TalkieUtil.get_talkie_talkie_version()
	set_settings_extended(show_settings_extended)
	update_restore_side_window_button()
	TalkieSlideHelper.side_window_settings_updated.connect(update_restore_side_window_button)
	
func update_restore_side_window_button() -> void:
	restore_side_window_button.visible = TalkieSlideHelper.is_side_window_restorable
...
func _load_values() -> void:
	change_locale(TalkiePreferences.language)
	TalkiePreferences.language_changed.emit()
	lang_option_button.select(Settings.get_locale_button_id(TalkiePreferences.language))
...
func _on_lang_option_button_item_selected(index: int) -> void:
	...
	TalkiePreferences.set_language(locale_new_short)
...
func _on_overall_volume_slider_value_changed(value: float) -> void:
	TalkiePreferencesClass.set_bus_volume(target_audio_bus, value)
	TalkiePreferences.set_audio_volume(value, false)
	
func _on_overall_volume_slider_drag_ended(value_changed: bool) -> void:
	if value_changed:
		TalkiePreferences.set_audio_volume(overall_volume_slider.value)

func _on_overall_volume_mute_button_toggled(toggled_on: bool) -> void:
	TalkiePreferencesClass.set_bus_muted(target_audio_bus, toggled_on)
	TalkiePreferences.set_audio_muted(toggled_on)
