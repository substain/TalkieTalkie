...
func _ready() -> void:
	...
	TTSlideHelper.side_window_settings_updated.connect(update_restore_side_window_button)
	
func update_restore_side_window_button() -> void:
	restore_side_window_button.visible = TTSlideHelper.is_side_window_restorable
...
func _load_values() -> void:
	change_locale(TTPreferences.language)
	TTPreferences.language_changed.emit()
	lang_option_button.select(Settings.get_locale_button_id(TTPreferences.language))
...
func _on_lang_option_button_item_selected(index: int) -> void:
	...
	TTPreferences.set_language(locale_new_short)
...
func _on_restore_side_window_button_pressed() -> void:
	TTSlideHelper.restore_side_window.emit()
