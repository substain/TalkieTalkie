...
func _ready() -> void:
	talkie_talkie_label.text = "TalkieTalkie " + TalkieUtil.get_talkie_talkie_version()
	godot_label.text = "Godot " + TalkieUtil.get_godot_version()
...
func get_rich_link(link: String) -> String:
	if !ProjectSettings.get_setting("talkietalkie/ui/enable_about_window_links", true) as bool:
		return link
