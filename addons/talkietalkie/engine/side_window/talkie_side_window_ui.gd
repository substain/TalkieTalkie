class_name TalkieSideWindowUI
extends CanvasLayer

signal preview_layout_settings_updated(rel_index: int, settings: TalkiePreviewLayoutSettings)

@export_category("internal nodes")
@export var slide_previews_by_rel_index: Dictionary[int, TalkieSlidePreview]
@export var preview_condensed_by_rel_index: Dictionary[int, TalkiePreviewCondensed]
@export var time_view: TalkieTimeView 
...
func set_preview_on_top(is_on_top_new: bool, rel_preview_index: int, update_condensed_button: bool = true, update_settings: bool = true) -> void:
	var target_preview: TalkieSlidePreview = slide_previews_by_rel_index[rel_preview_index]
...
func on_resize(new_window_size: Vector2) -> void:
	for slide_preview: TalkieSlidePreview in slide_previews_by_rel_index.values():
...
func load_side_window_layout_settings(preview_layout_settings_by_rel_index: Dictionary[int, TalkiePreviewLayoutSettings]) -> void:
	for rel_preview_index: int in slide_previews_by_rel_index.keys():
...
func set_preview_layouts_locked(is_layout_locked: bool) -> void:
	for rel_preview_index: int in slide_previews_by_rel_index.keys():
		var slide_preview: TalkieSlidePreview = slide_previews_by_rel_index[rel_preview_index]
