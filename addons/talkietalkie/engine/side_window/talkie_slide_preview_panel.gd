class_name TalkieSlidePreview extends Window
...
var current_slide: TalkieSlide = null
var original_slide: TalkieSlide = null
...
var preview_layout_settings: TalkiePreviewLayoutSettings = TalkiePreviewLayoutSettings.new() #these settings may be updated / saved
var preview_theme_settings: PreviewThemeSettings = PreviewThemeSettings.new() #readonly

var initialized: bool = false

func _enter_tree() -> void:
	preview_layout_settings = TalkiePreviewLayoutSettings.load_default(offset_from_current_slide)
	start_pos = position
	TalkieSlideHelper.context_initialized.connect(_on_slide_context_initialized)
	TalkieSlideHelper.slide_changed.connect(_on_slide_changed)
	if offset_from_current_slide == 0:
		TalkieSlideHelper.progress_changed.connect(_on_slide_progress_changed)
	...
	if TalkieSlideHelper.has_context:
		_on_slide_context_initialized()
		load_slide_by_index(TalkieSlideHelper.current_slide._order_index)
	
func _ready() -> void:
	size_changed.connect(_on_size_changed)
		
func _on_slide_context_initialized() -> void:
	slide_size = TalkieSlideHelper.get_context().slide_size
	target_resolution = slide_size.x / slide_size.y

func _on_slide_changed(new_slide: TalkieSlide) -> void:
...
func load_slide_by_index(new_index: int) -> void:
	...
	if slide_index < 0 || slide_index >= TalkieSlideHelper.slide_controller.slide_instances.size():
		update_window_by_slide()
		return
		
	original_slide = TalkieSlideHelper.slide_controller.slide_instances.get(slide_index)
	var packed_scene: PackedScene = TalkieSlideHelper.get_context().slide_templates[slide_index]
...
func load_slide_by_packed_scene(packed_scene: PackedScene) -> void:
	current_slide = packed_scene.instantiate() as TalkieSlide
	...
	if current_slide is TalkieSceneSlide && (current_slide as TalkieSceneSlide).preview_info_elements != null && !(current_slide as TalkieSceneSlide).preview_info_elements.is_empty():
		for info_element: Node in (current_slide as TalkieSceneSlide).preview_info_elements:
...
func do_resize(update_this_window_size: bool) -> void:
	if !TalkieSlideHelper.has_context: # this ensures slide_size is valid
		return
...
func _on_slide_progress_changed(_new_progress: float) -> void:
	...
	for elem: Variant in progress_elements.keys():
		...
		elif elem is TalkieSlideAnimation:
			handle_progress_elem_animation(elem as TalkieSlideAnimation, is_elem_visible)
			...
			if is_elem_visible:
				if node is CanvasItem:
					...
					if copied_node != null:
						...
						TalkieUtil.tt_printr("could not get node from path: %s" % original_slide.get_path_to(node))
			else:
				if node is CanvasItem:
					var copied_node: CanvasItem = get_copy_node_with_same_path(node) as CanvasItem
					if copied_node != null:
...
func handle_progress_elem_animation(anim: TalkieSlideAnimation, is_elem_visible: bool) -> void:
	#var source_anim: SlideAnimation = elem as SlideAnimation
	if !(current_slide is TalkieAnimSlide) || !(original_slide is TalkieAnimSlide):
		return
	var anim_pos: int = (original_slide as TalkieAnimSlide).animations.find(anim)
	var target_anim_copy: TalkieSlideAnimation = (current_slide as TalkieAnimSlide).animations[anim_pos]
...
func update_settings() -> void:
	...
	#TalkieUtil.tt_debug("updated settings...")

func load_settings(preview_layout_settings_new: TalkiePreviewLayoutSettings) -> void:
	var target_pos: Vector2 = preview_layout_settings_new.position

	target_pos = TalkieSideWindow.constrain_to_bounds(target_pos, preview_layout_settings_new.size, Rect2(Vector2(0, min_y_pos), parent_window.size), 0)
	await get_tree().process_frame
