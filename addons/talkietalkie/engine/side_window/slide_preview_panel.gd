...
func _enter_tree() -> void:
	...
	start_pos = position
	TTSlideHelper.context_initialized.connect(_on_slide_context_initialized)
	TTSlideHelper.slide_changed.connect(_on_slide_changed)
	if offset_from_current_slide == 0:
		TTSlideHelper.progress_changed.connect(_on_slide_progress_changed)
	...
	if TTSlideHelper.has_context:
		_on_slide_context_initialized()
		load_slide_by_index(TTSlideHelper.current_slide._order_index)
	
func _ready() -> void:
	size_changed.connect(_on_size_changed)
		
func _on_slide_context_initialized() -> void:
	slide_size = TTSlideHelper.get_context().slide_size
...
func load_slide_by_index(new_index: int) -> void:
	...
	if slide_index < 0 || slide_index >= TTSlideHelper.slide_controller.slide_instances.size():
		update_window_by_slide()
		return
		
	original_slide = TTSlideHelper.slide_controller.slide_instances.get(slide_index)
	var packed_scene: PackedScene = TTSlideHelper.get_context().slide_templates[slide_index]
...
func do_resize(update_this_window_size: bool) -> void:
	if !TTSlideHelper.has_context: # this ensures slide_size is valid
		return
