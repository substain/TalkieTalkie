class_name TalkieAnimSlide extends TalkieSceneSlide
...
var animations: Array[TalkieSlideAnimation]
...
func _ready() -> void:
	...
	animations.sort_custom(TalkieUtil.compare_by_sort_order)
	anim_steps = animations.size()
	_prepare_progress_elements()
	reset()
	
func _prepare_progress_elements() -> void:
	for animation: TalkieSlideAnimation in animations:
...
func continue_slide() -> bool:
	...
	if target_anim_step > 0:
		var animation: TalkieSlideAnimation = animations[target_anim_step-1]
		animation.skip_to_finish()
		progress_elements[animation as Variant] = true
	
	var current_animation: TalkieSlideAnimation = animations[target_anim_step]
...
func set_progress(relative_progress: float) -> bool:
	...
	for animation_index: int in anim_steps:
		var anim: TalkieSlideAnimation = animations[animation_index]
	...
	var target_anim: TalkieSlideAnimation = animations[requested_step]
...
static func collect_anims_in_children(node: Node, current_index: int) -> Array[TalkieSlideAnimation]:
	var res: Array[TalkieSlideAnimation] = []
	if node is TalkieSlideAnimation:
		res.append(node as TalkieSlideAnimation)
		(node as TalkieSlideAnimation).tree_index = current_index
		current_index += 1
		
	for child: Node in node.get_children():
		var children_nodes: Array[TalkieSlideAnimation]= collect_anims_in_children(child, current_index)
