class_name TalkieSlide extends Control
...
func _update_progress(new_progress: float) -> void:
	...
	if is_currently_active_slide:
		TalkieSlideHelper.progress_changed.emit(_current_progress)
...
func set_in_transition_override(_new_in_transition_override: TalkieTransition) -> void:
	pass
