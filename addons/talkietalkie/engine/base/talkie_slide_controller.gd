class_name TalkieSlideController
extends Node
...
var slide_instances: Array[TalkieSlide]
var slide_index: int = 0
var current_slide: TalkieSlide
var default_transition: TalkieTransition

var transition_tween: Tween = null
var last_from_slide: TalkieSlide = null
var last_transition: TalkieTransition

func init(slide_instances_new: Array[TalkieSlide], default_transition_new: TalkieTransition, manual_navigation_stops_slideshow_new: bool, has_faulty_configuration: bool) -> void:
	...
	TalkieSlideHelper.slide_controller = self
...
func do_continue(automatic: bool = false) -> void:
	...
	TalkieSlideHelper.ui.control_bar.set_slide_progress_flags(current_slide.is_at_start(), current_slide.is_finished())
	if !was_finished:
		return
		
	var previous_slide: TalkieSlide = current_slide
	var has_next_slide: bool = change_slide(1)
	if !has_next_slide:
		return

	var used_transition: TalkieTransition = current_slide.get_in_transition_override() if current_slide.get_in_transition_override() != null else default_transition
...
func skip_to_current_slide_full() -> void:
	current_slide.show_full()
	TalkieSlideHelper.ui.control_bar.set_slide_progress_flags(current_slide.is_at_start(), true)
	show_only_current_slide()
	
func transition_to_slide(from_slide: TalkieSlide, to_slide: TalkieSlide, transition: TalkieTransition) -> void:
...
func show_only_current_slide() -> void:
	...
	match TalkieSlideHelper.get_context().slide_context_type:
		TalkieSlideContext.SlideContextType.CONTROL:
			show_only_current_slide_control()
		TalkieSlideContext.SlideContextType.NODE_2D:
			show_only_current_slide_2d()
		TalkieSlideContext.SlideContextType.NODE_3D:
...
func show_only_current_slide_2d() -> void:
	TalkieSlideHelper.get_context_2d().camera.global_position = TalkieSlideHelper.get_context_2d().get_slide_center_position(slide_instances[slide_index])

func show_only_current_slide_3d() -> void:
	TalkieSlideHelper.get_context_3d().camera.global_position = TalkieSlideHelper.get_context_3d().get_slide_center_position(slide_instances[slide_index])
...
func set_current_slide(slide_new: TalkieSlide) -> void:
	...
	TalkieSlideHelper.current_slide = slide_new
	current_slide.reset()
	update_ui_slide_status()
	current_slide.activate_slide.emit()
	TalkieSlideHelper.slide_changed.emit(current_slide)
...
func set_slide_index(new_index: int) -> void:
	...
	TalkiePreferences.set_presentation_progress(slide_index, get_tree().current_scene.scene_file_path)

	TalkieSlideHelper.ui.set_current_slide(slide_index)
	
func start_auto_slideshow(update_ui: bool = true) -> void:
	auto_slideshow_timer.start()
	if update_ui:
		TalkieSlideHelper.ui.set_auto_slideshow_active(true)

func stop_auto_slideshow(update_ui: bool = true) -> void:
	auto_slideshow_timer.stop()
	if update_ui:
		TalkieSlideHelper.ui.set_auto_slideshow_active(false)
		
func update_ui_slide_status() -> void:
	TalkieSlideHelper.ui.control_bar.set_slide_status_flags(slide_index == 0, slide_index == slide_instances.size()-1)
	TalkieSlideHelper.ui.control_bar.set_slide_progress_flags(slide_instances[slide_index].is_at_start(), slide_instances[slide_index].is_finished())
