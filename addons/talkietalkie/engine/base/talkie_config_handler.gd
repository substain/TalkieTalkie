class_name TalkieXConfigHandler
extends Object
...
class Config:
	...
	# General
	var remember_slide_on_close: bool
	...
	var show_audio_for_bus: StringName
	...
	static func get_default() -> Config:
		...
		default_config.remember_slide_on_close = false
		...
		default_config.show_audio_for_bus = &"Master"
...
static func load_config() -> Config:
	...
	var default_filepath: String = TalkieSetup.get_addon_path() + ADDON_CFG_PATH
...
static func _update_config_from_cfg_file(config: Config, config_file: ConfigFile) -> void:
	...
	# General
	var remember_slide_on_close: String = _load_value(config_file, "General", "remember_slide_on_close")
	if !remember_slide_on_close.is_empty():
		config.remember_slide_on_close = str_to_var(remember_slide_on_close)
...
#
	...
	var show_audio_for_bus: String = _load_value(config_file, "Settings", "show_audio_for_bus")
	if !show_audio_for_bus.is_empty():
		config.show_audio_for_bus = show_audio_for_bus
