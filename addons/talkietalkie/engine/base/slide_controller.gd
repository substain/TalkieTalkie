...
func init(slide_instances_new: Array[Slide], default_transition_new: Transition, manual_navigation_stops_slideshow_new: bool, has_faulty_configuration: bool) -> void:
	...
	TTSlideHelper.slide_controller = self
...
func do_continue(automatic: bool = false) -> void:
	...
	TTSlideHelper.ui.control_bar.set_slide_progress_flags(current_slide.is_at_start(), current_slide.is_finished())
	if !was_finished:
		return
...
func skip_to_current_slide_full() -> void:
	current_slide.show_full()
	TTSlideHelper.ui.control_bar.set_slide_progress_flags(current_slide.is_at_start(), true)
	show_only_current_slide()
...
func show_only_current_slide() -> void:
	...
	match TTSlideHelper.get_context().slide_context_type:
...
func show_only_current_slide_2d() -> void:
	TTSlideHelper.get_context_2d().camera.global_position = TTSlideHelper.get_context_2d().get_slide_center_position(slide_instances[slide_index])

func show_only_current_slide_3d() -> void:
	TTSlideHelper.get_context_3d().camera.global_position = TTSlideHelper.get_context_3d().get_slide_center_position(slide_instances[slide_index])
...
func set_current_slide(slide_new: Slide) -> void:
	...
	TTSlideHelper.current_slide = slide_new
	current_slide.reset()
	update_ui_slide_status()
	current_slide.activate_slide.emit()
	TTSlideHelper.slide_changed.emit(current_slide)
...
func set_slide_index(new_index: int) -> void:
	...
	TTPreferences.set_presentation_progress(slide_index, get_tree().current_scene.scene_file_path)

	TTSlideHelper.ui.set_current_slide(slide_index)
	
func start_auto_slideshow(update_ui: bool = true) -> void:
	auto_slideshow_timer.start()
	if update_ui:
		TTSlideHelper.ui.set_auto_slideshow_active(true)

func stop_auto_slideshow(update_ui: bool = true) -> void:
	auto_slideshow_timer.stop()
	if update_ui:
		TTSlideHelper.ui.set_auto_slideshow_active(false)
		
func update_ui_slide_status() -> void:
	TTSlideHelper.ui.control_bar.set_slide_status_flags(slide_index == 0, slide_index == slide_instances.size()-1)
	TTSlideHelper.ui.control_bar.set_slide_progress_flags(slide_instances[slide_index].is_at_start(), slide_instances[slide_index].is_finished())
