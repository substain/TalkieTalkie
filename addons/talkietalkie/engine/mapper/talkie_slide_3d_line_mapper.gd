class_name TalkieSlide3DLineMapper
extends Node3D
...
func _ready() -> void:
	...
	if TalkieSlideHelper.get_context() != null:
		map_slides_to_3d()
	else:
		TalkieSlideHelper.context_initialized.connect(map_slides_to_3d)
		
func map_slides_to_3d() -> void:
	var slide_context: TalkieSlideContext3D = TalkieSlideHelper.get_context_3d()
	...
	var children: Array[TalkieSlide] = TalkieUtil.collect_slides_in_children(target_parent)
	var direction: Vector3 = slide_separation.normalized()
	var slide_center_locations: Dictionary[Vector3, TalkieSlide] = {}

	for i: int in children.size():
		var child: TalkieSlide = children[i]
		...
		slide_center_locations[new_pos + TalkieUtil.to_vec3(slide_context.slide_center_offset, 0)] = child 
