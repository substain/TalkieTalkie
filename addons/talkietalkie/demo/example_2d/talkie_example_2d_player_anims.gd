class_name TalkieExample2DPlayerAnims 
...
func _ready() -> void:
	TalkieSlideHelper.pointing_at_pos.connect(_on_pointing_at_pos)
	TalkieSlideHelper.stop_drawing.connect(_on_stop_pointing)
...
func _on_pointing_at_pos(point_pos: Vector2, _is_drawing: bool, _draw_properties: TalkieDrawProperties) -> void:
	is_pointing = true
