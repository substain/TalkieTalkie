...
@export var move_transition: TalkieMoveTransition2D
...
@export var anim_player: TalkieExample2DPlayerAnims
...
func _ready() -> void:
	...
	TalkieSlideHelper.slide_changed.connect(teleport_to_slide)
...
func handle_stuck_player(delta: float) -> void:
	...
	if _unstuck_progress >= unstuck_duration:
		...
		if set_slide_progress_full_on_unstuck:
			TalkieSlideHelper.slide_controller.set_current_slide_progress(1.0)
...
func update_camera() -> void:
	var slide_context: TalkieSlideContext2D = TalkieSlideHelper.get_context_2d()
	...
	var target_slide: TalkieSlide = slide_context.slide_center_locations[closest_slide_pos]
	if target_slide == current_slide:
		return
		
	move_transition.start_transition(current_slide, target_slide)
	current_slide = target_slide
	TalkieSlideHelper.slide_controller.set_slide(target_slide.get_order_index())
	
func teleport_to_slide(slide: TalkieSlide) -> void:
...
func walk_to_slide(target_pos: Vector2, slide_context: TalkieSlideContext2D) -> void:
